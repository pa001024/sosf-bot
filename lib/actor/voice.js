...
export class VoiceActor {
	...
	async displayLyric(chan) {
		...
		let lrc = await CloudMusic.getLyric(muInfo.id);
		if (lrc.uncollected) {
			chan.send(tplHead + "*无歌词*")
				.then(m => this.lyricSlide = m);
			setTimeout(() => {
				if (this.session) {
					this.session.on('end', () => {
						if (this.lyricSlide) {
							this.lyricSlide.edit(tplHead + `**- END -**`)
							this.lyricSlide && this.lyricSlide.delete(1e3);
						}
					});
				}
			}, 2e3);
		...
		player.on('update', (time, txts) => {
			if (txts && txt[0].trim() == "") return;
			if (this.lyricSlide) {
				this.lyricSlide.edit(tplHead + txts.map(v => `**♪ ${v} ♪**`).join("\n"));
			} else {
				chan.send(tplHead + txts.map(v => `**♪ ${v} ♪**`).join("\n"))
					.then(m => this.lyricSlide = m);
			}
	...
	async playNext() {
		...
		if (this.enableLyric && this.playChannel) this.displayLyric(this.playChannel);
