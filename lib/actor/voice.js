...
export class VoiceActor {
	...
	displayLyric(chan, muInfo, lrc) {
		let slide, slides = [],
			...
			addStopReact = (m) => {
				m.react("⏹");
				const filter = (reaction, user) => reaction.emoji.name === '⏹' && user.id !== this.app.client.user.id;
				const col = m.createReactionCollector(filter, { time: muInfo.duration });
				col.on('collect', ac => { col.stop(); this.stop(); });
			};
	...
	act(msg) {
		if (!this.enableTTS || this.voiceconn == null || msg.tts) return false;
